...
var removeCode = function ( frag ) {
    var codes = frag.querySelectorAll( 'code' );
    Array.prototype.filter.call( codes, function ( el ) {
        return !getNearest( el.parentNode, 'CODE' );
    }).forEach( function ( el ) {
        replaceWith( el, empty( el ) );
    });
    return frag;
...
var allowedBlock = /^(?:A(?:DDRESS|RTICLE|SIDE)|BLOCKQUOTE|CODE|CAPTION|D(?:[DLT]|IV)|F(?:IGURE|OOTER)|H[1-6]|HEADER|L(?:ABEL|EGEND|I)|O(?:L|UTPUT)|P(?:RE)?|SECTION|T(?:ABLE|BODY|D|FOOT|H|HEAD|R)|UL)$/;
...
proto.makeCode = command( 'modifyBlocks', makeCode );
proto.removeCode = command( 'modifyBlocks', removeCode );
